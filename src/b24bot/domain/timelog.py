...
# Быстрые кнопки — часы рабочего дня, от одного до восьми (09.09.2026, по заказчику).
# Прежний набор (15 м, 30 м, 1, 2, 4, 8 ч) не покрывал 3, 5, 6 и 7 часов, то есть
# половину смены нельзя было списать ни одной кнопкой. Всё, что не целый час,
# теперь набирается через «Другое» — и там же живут минуты.
PRESETS: tuple[int, ...] = tuple(hours * 3600 for hours in range(1, 9))
...
def parse_duration(text: str) -> int:
    """«1ч30м», «1:30», «90м», «1.5ч», «90», «1,5» → секунды.

    Два правила на голое число, и они не спорят друг с другом:

    * **целое — это МИНУТЫ.** Час бы значил, что опечатка в один символ
      («2» вместо «2м») ошибается в шестьдесят раз в сторону завышения,
      а завышенные трудозатраты у клиента дороже заниженных: их оспаривают,
      а не прощают;
    * **дробное — это ЧАСЫ.** Дробных минут в учёте не бывает — минимальное
      списание и так минута, — поэтому «1,5» может означать только полтора
      часа. Запятую случайно не набирают, так что и опечатки здесь нет.
    """
    ...
    if raw.isdigit():
        return checked(int(raw) * 60)
    if re.fullmatch(r"\d+\.\d+", raw):
        return checked(round(float(raw) * 3600))
