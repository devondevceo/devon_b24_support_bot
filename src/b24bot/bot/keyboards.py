...
def force_reply(placeholder: str = "") -> dict[str, Any]:
    """Поле ответа, наведённое на наше сообщение.

    Единственная разрешённая форма свободного ввода в группе (docs/30-bot-spec.md
    §0.4): ответ приходит реплаем, то есть с указанием, НА ЧТО он отвечает. Приём
    «первого некомандного сообщения» съедал бы обычные реплики коллегам.

    `selective` намеренно НЕ ставится: он показал бы поле ответа только тому, кого
    упомянули в тексте, а упоминать человека в общем чате ради этого — лишний пинг.
    Ответить может любой участник, и это не дыра: время списывается тому, кто
    ответил, его личным токеном, ровно как при `/time`.
    """
...
def timelog_menu(items: list[tuple[str, str]], back: str, ask: str,
                 app_url: str | None = None, *,
                 private: bool = False) -> dict[str, Any]:
    """Быстрые длительности по три в ряд, девятой — «Другое».

    Восемь часов рабочего дня плюс произвольная длительность дают ровно три ряда
    по три: набор, в котором ничего не приходится искать глазами.

    Свободный ввод открывается кнопкой «Другое» и принимается только реплаем
    (§0.4 спеки) — бот не умеет ждать «следующего сообщения» от одного человека,
    не перехватывая чужие реплики.
    """
    rows: Rows = []
    row: list[Button] = []
    for token, label in [*items, (ask, "✏️ Другое")]:
        row.append(cb("tm", token, label) if private else cb("tl", token, label))
        if len(row) == 3:
            rows.append(row)
            row = []
    if row:
        rows.append(row)
    if app_url:
        # Не «другое время» — оно теперь тут же, кнопкой. В приложении есть то,
        # чего в чате нет и не будет: дата, за которую списывают, и комментарий.
        rows.append([url_button("🧩 За другой день — в приложении", app_url)])
