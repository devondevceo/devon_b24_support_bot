...
def _scope_form(session: str, active: str, scope_kind: str, scope_id: int,
    ...
    return (
        ...
        f"с кнопками.</p>"
        f'<p class="hint">{esc_html(_instant_note())}</p>'
        f"</form>")


def _instant_note() -> str:
    """Что группировка не задерживает никогда.

    Исключение обязано быть названо прямо на экране: настройка, которая молча
    действует не на всё, читается как поломка ровно у того, кто её включил.
    """
    names = [e.label for e in notifications.EVENTS if e.instant]
    return (f"Не ждёт группировки: {', '.join(names)} — появление новой задачи "
            f"это начало работы, и в сводке оно приходило бы поздно.")
