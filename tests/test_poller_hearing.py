...
2. **Причина глухоты видна человеку** (`tg_bots.poll_error`, миграция 0022) — один
...
def test_deafness_outlasts_the_short_poll_fallback() -> None:
    ...
    пределу захода плюс паузы; предел глухоты обязан быть длиннее, иначе
    healthcheck краснел бы на сети, где бот работает.
    """
    limit = (tg.deadline("getUpdates", {"timeout": poller_mod.POLL_TIMEOUT})
             + poller_mod.HARD_MARGIN)
    fallback = poller_mod.FALLBACK_AFTER * (limit + poller_mod.ERROR_SLEEP)
...
async def test_state_statements_match_the_schema(db: Any) -> None:
    """Запросы поллера и экрана ходят в настоящие колонки миграции 0022.
