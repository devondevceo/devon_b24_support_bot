...
# Выкатка держит на диске только текущий и KEEP_PREV_IMAGES предыдущих образов
# (по умолчанию два, docs/80-deploy.md §8.11) — отсутствие более старого здесь
# норма, а не поломка.
docker image inspect "$target" >/dev/null 2>&1 \
  || fail "образа $target нет локально (старые убирает выкатка, docs/80-deploy.md §8.11); docker pull $target (нужен логин в реестр)"
log "откат на $target"
