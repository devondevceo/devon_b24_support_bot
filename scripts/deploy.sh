...
# Кэш сборки и висячие образы старше недели. Обе команды идут по всему демону, то
# есть и по соседям, — и ровно поэтому берут только ничьё: кэш, которым неделю никто
# не пользовался, и образы без тега. `image prune` БЕЗ -a: с -a он снёс бы
# неиспользуемые образы соседей по машине. Свои теги — отдельной уборкой ниже.
prune_stale() {
...
# --- 1. место на диске ------------------------------------------------------
# Уборка стоит в конце выкатки, а проверка — в начале, и между выкатками машину
# заполняют соседи: их сборки копят кэш на том же демоне. 14.09.2026 выкатку
# пришлось готовить ручной чисткой, 23.09 она упала на тех же 86%, ничего не тронув.
# Поэтому сначала та же уборка, что и после выкатки (шаг 9), и только потом отказ —
# ничего сверх уже принятого, и только когда предел превышен.
used=$(disk_used)
if [ "$used" -gt "$DISK_MAX_PCT" ]; then
  log "диск: занято ${used}% при пределе ${DISK_MAX_PCT}% — убираю кэш сборки и висячие образы старше недели"
  prune_stale
  used=$(disk_used)
  [ "$used" -le "$DISK_MAX_PCT" ] \
    || fail "на диске занято ${used}%, предел ${DISK_MAX_PCT}%: уборка кэша не помогла, место заняло другое (docs/80-deploy.md §8.12)"
fi
...
# Свои старые теги — отдельным шагом и только под своим именем: общая уборка
# (`prune_stale`) тегированных образов не трогает.
