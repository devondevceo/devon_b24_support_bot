...
| — | `digest BOOLEAN`, `digest_text TEXT` | группировка уведомлений, миграция `0015` |
...
**Уровень выигрывает целиком, а не по одному коду.** Если у чата есть хоть одна своя строка,
берётся весь его набор, а коды, которых в нём нет, — из системных дефолтов, а не из проекта.
Иначе чат, выключивший всё, получал бы новое событие с уровня выше: «выключил всё» означало
бы «всё, кроме того, что добавят потом». Реализация — `notifications.resolve_rows`.

### Группировка уведомлений (миграция `0015`)
