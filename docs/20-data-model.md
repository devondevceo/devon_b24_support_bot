...
- Telegram `chat_id` фигурирует **только** в `tg_chats.chat_id`. Все прочие таблицы ссылаются
  на суррогатный `tg_chats.id`. Миграция группы в супергруппу при этом **не** один `UPDATE`,
  как обещал первый вариант этого документа: номера сообщений у чатов свои, а `chat_ref`
  входит в ключи рядом с номером сообщения — новому чату нужна новая строка (§4.1).
...
CREATE TABLE tg_bots (
  ...
  -- Слышит ли поллер Telegram (миграция 0022). heard_at — последний ответ на
...
CREATE TABLE tg_chats (
  ...
  migrated_to   BIGINT REFERENCES tg_chats(id) ON DELETE SET NULL,  -- SET NULL: миграция 0021
  first_seen_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  claimed_at    TIMESTAMPTZ
);
CREATE UNIQUE INDEX ux_tg_chats__chat_id ON tg_chats (chat_id) WHERE status <> 'migrated';
CREATE INDEX ix_tg_chats__migrated ON tg_chats (chat_id) WHERE status = 'migrated';  -- 0021
```

> Строка создаётся **только** из фактического апдейта Telegram (`my_chat_member` или
> сообщение) — или из ответа Telegram о том, что группа стала супергруппой (§4.1): это тоже
> слово самого Telegram, а не ввод человека. Ручной ввод `chat_id` в панели запрещён: иначе
> теннант B занимает чат клиента теннанта A и навсегда блокирует привязку, а по тексту ошибки
> узнаёт, обслуживается ли этот чат конкурентом через наш сервис. Ошибка конфликта —
> обезличенная.
...
### 4.1 Группа стала супергруппой: надгробие и преемник (миграция `0021`, 23.09.2026)

Обычная группа Telegram становится супергруппой сама — и получает новый `chat_id`. Переезд
делает `domain/chat_migration.py` (поведение — docs/30-bot-spec.md §10.1); здесь — что он
делает со схемой.

**Новому чату — новая строка.** Первый вариант этого документа обещал «один `UPDATE`
`chat_id`», и для привязок это было бы правдой. Но номер сообщения Telegram уникален только
внутри чата, а `chat_ref` стоит рядом с номером сообщения в двух местах:

| Ключ | Что сломалось бы при общем `chat_ref` |
|---|---|
| `tgsrc-<chat_ref>-<message_id>` — ключ идемпотентности создания задачи, тег задачи в Битриксе (И-10) | `/task` на новое сообщение с номером старого отвечал бы «уже создана» чужой задачей. Тег на портале не почистить |
| `tg_message_links (chat_ref, message_id, kind)` | новое сообщение совпало бы со связью старого |

Поэтому старая строка становится **надгробием**: `status='migrated'`, `migrated_to` —
преемник, `chat_id` и `tenant_id` — прежние. К преемнику переезжают заявка теннанта,
`chat_bindings` (вместе с id — и с настройками уведомлений уровня привязки, §9),
`tg_topics`, `outbox` в `pending`/`sending`, живые `callback_tokens` и отметки
`reminder_marks` со scope `chat`. У надгробия остаются `tg_message_links` и отменённые
`survey_sessions`. Каждая таблица со ссылкой на `tg_chats(id)` названа в
`chat_migration.MOVES` или `STAYS`, и страж сверяет это с каталогом: **новая таблица с
`chat_ref` обязана попасть в один из двух списков**, иначе её строки молча остались бы у
надгробия.

Надгробие по-прежнему упраздняет свой `chat_id`: регистрация чата (`dispatch._register_chat`)
не заводит его заново ничейной строкой. Поиск идёт на каждом апдейте, а уникальный индекс
частичный и надгробий не видит — отсюда `ix_tg_chats__migrated`.

**`migrated_to` — `ON DELETE SET NULL`.** До `0021` ссылка была NO ACTION и никогда не
заполнялась. Заполненная, она держала бы удаление преемника: надгробие ничейной группы
(`tenant_id IS NULL`) указывает на чат, который потом заявил теннант, — и чистка этого
теннанта (`lifecycle.purge_due`) падала бы на ссылке каждые сутки, а обещанный юрдокументами
срок удаления данных молча не соблюдался бы. Надгробие без преемника — всё ещё надгробие.
...
**Не всякая новость копится.** Коды с `Event.instant` (сейчас — `task.created`) ставятся
в очередь готовыми к отправке при любом интервале: группировка бережёт чат от потока
правок по известной задаче, а не откладывает появление новой.
