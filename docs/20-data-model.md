...
RLS включён с двумя сознательными отступлениями от плана выше — оба меняют
механику, но не гарантию:

1. **`FORCE ROW LEVEL SECURITY` вместо отдельной роли `b24bot_app`.** Приложение
   продолжает подключаться владельцем базы, но FORCE подчиняет политике и
   владельца. Отдельная login-роль потребовала бы вторых кредов в `.env` и ручной
   операции на сервере; FORCE даёт тот же эффект без новых секретов. «Ролью
   обслуживания» служит не `BYPASSRLS`, а GUC: соединение, НЕ объявившее
   `app.rls='enforce'`, работает вне политики — это alembic, psql руками и старый
   образ при новой схеме (вперёд-совместимость буквально). Инъекция через asyncpg
   объявление снять не может: extended-протокол не пускает вторую команду.
2. **Резолверы — питоновские скоупы, а не `SECURITY DEFINER`-функции.** Под FORCE
   определитель функции тоже подчинён политике, и SQL-резолверы потеряли смысл.
   Их роль исполняет `system_scope()` из `db/pool.py`: узкий блок вокруг ровно
   одного запроса «найди теннанта по недоверенному идентификатору» (`member_id`,
...
CI гоняет все живые тесты с включённым флагом, прод включает его отдельным шагом
(docs/80-deploy.md §9). Пока флаг выключен, фасад не делает ни одного лишнего
запроса.
